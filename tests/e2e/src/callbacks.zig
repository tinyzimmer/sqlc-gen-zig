...
const enums = @import("gen/callbacks/enums.zig");
const OrderQueries = @import("gen/callbacks/orders.sql.zig");
const OrderQuerier = OrderQueries.PoolQuerier;
const UserQueries = @import("gen/callbacks/users.sql.zig");
const UserQuerier = UserQueries.PoolQuerier;
const TestDB = @import("testdb.zig");

test "callbacks - one field queries" {
...
//     const querier = UserQuerier.init(test_db.pool);
//     const empty_users = try querier.getUserIDsByRole(allocator, .admin);
...
//     const user_ids = try querier.getUserIDsByRole(allocator, .admin);
//     defer {
//         if (user_ids.len > 0) {
...
//     const querier = UserQuerier.init(test_db.pool);
//     const empty = try querier.getUserIDsBySalaryRange(allocator, 0, 1000);
...
//     const users = try querier.getUserIDsBySalaryRange(allocator, 0, 1000);
//     defer {
//         if (users.len > 0) {
//             allocator.free(users);
//         }
...
//     const ip_users = try querier.getUserIDsByIPAddress(allocator, "192.168.1.1");
//     defer {
//         if (ip_users.len > 0) {
...
//     const empty = try querier.getOrders(allocator);
//     try expectEqual(0, empty.len);
...
//     const order = try querier.getOrders(allocator);
//     defer {
//         if (order.len > 0) {
//             for (order) |o| {
//                 o.deinit();
