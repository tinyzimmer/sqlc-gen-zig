...
test "sqlite(managed): one field queries" {
...
test "sqlite(managed): many field queries" {
    const expectEqual = std.testing.expectEqual;
    const allocator = std.testing.allocator;
    ...
    const querier = UserQuerier.init(allocator, test_db.pool);
    const empty_users = try querier.getUserIDsBySalaryRange(1000, 2000);
    ...
    const user_ids = try querier.getUserIDsBySalaryRange(1000, 2000);
...
test "sqlite(managed): one struct queries" {
...
test "sqlite(managed): many struct queries" {
    const expect = std.testing.expect;
    const expectEqual = std.testing.expectEqual;
    const expectEqualStrings = std.testing.expectEqualStrings;
...
test "sqlite(managed): partial struct returns" {
    const expectEqual = std.testing.expectEqual;
    const expectEqualStrings = std.testing.expectEqualStrings;
